...
static void
sb_str (char *buf, int *pos, const char *s)
{
    while (*s)
        buf[(*pos)++] = *s++;
    buf[*pos] = '\0';
...
static void
sb_hex (char *buf, int *pos, zetui_u32 cp)
{
...
static zetui_style_t
mk_style (zetui_i32 fg, zetui_i32 bg, zetui_u32 attrs)
{
    zetui_style_t s;
    s.fg = fg;
    s.bg = bg;
...
static void
panel_colors (zetui_ctx_t *ctx, int x, int y, int w, int h)
{
...
static void
panel_attrs (zetui_ctx_t *ctx, int x, int y, int w, int h)
{
    zetui_style_t hdr, s;
    int row;
    ...
    zetui_draw_str (ctx, x, y,
                    "Text attributes (terminal support may vary):", hdr);
...
#define ATROW(label, fg, bg, attr)                                             \
    ...
    ATROW ("NONE       normal text -- baseline", ZETUI_COLOR_DEFAULT,
           ZETUI_COLOR_DEFAULT, ZETUI_ATTR_NONE);
    ATROW ("BOLD       heavier weight", ZETUI_COLOR_DEFAULT,
           ZETUI_COLOR_DEFAULT, ZETUI_ATTR_BOLD);
    ATROW ("DIM        reduced intensity", ZETUI_COLOR_DEFAULT,
           ZETUI_COLOR_DEFAULT, ZETUI_ATTR_DIM);
    ATROW ("ITALIC     slanted (if supported)", ZETUI_COLOR_DEFAULT,
           ZETUI_COLOR_DEFAULT, ZETUI_ATTR_ITALIC);
    ATROW ("UNDERLINE  single underline", ZETUI_COLOR_DEFAULT,
           ZETUI_COLOR_DEFAULT, ZETUI_ATTR_UNDERLINE);
    ATROW ("BLINK      slow blink (if supported)", ZETUI_COLOR_DEFAULT,
           ZETUI_COLOR_DEFAULT, ZETUI_ATTR_BLINK);
    ATROW ("REVERSE    swap fg/bg", ZETUI_COLOR_WHITE, ZETUI_COLOR_BLACK,
           ZETUI_ATTR_REVERSE);
    ATROW ("HIDDEN     invisible text (select to reveal)", ZETUI_COLOR_WHITE,
           ZETUI_COLOR_DEFAULT, ZETUI_ATTR_HIDDEN);
    ATROW ("STRIKE     strikethrough", ZETUI_COLOR_DEFAULT, ZETUI_COLOR_DEFAULT,
           ZETUI_ATTR_STRIKE);
...
#undef ATROW
    ...
    zetui_draw_str (ctx, x, row++, "  BOLD | REVERSE    (red fg, yellow bg)",
                    s);
...
static void
panel_boxes (zetui_ctx_t *ctx, int x, int y, int w, int h)
{
    ...
    hdr = mk_style (ZETUI_COLOR_BRIGHT_WHITE, ZETUI_COLOR_DEFAULT,
                    ZETUI_ATTR_BOLD);
    plain
        = mk_style (ZETUI_COLOR_DEFAULT, ZETUI_COLOR_DEFAULT, ZETUI_ATTR_NONE);
    ...
    zetui_set_cell (ctx, bx, y + 4,
                    zetui_cell_make (zetui_box_light[ZETUI_BOX_LT], plain));
    zetui_draw_hline (ctx, bx + 1, y + 4, bw - 2, zetui_box_light[ZETUI_BOX_H],
                      plain);
    zetui_set_cell (ctx, bx + bw - 1, y + 4,
                    zetui_cell_make (zetui_box_light[ZETUI_BOX_RT], plain));
    ...
    zetui_draw_hline (ctx, bx + 1, y + 4, bw - 2, zetui_box_heavy[ZETUI_BOX_H],
                      plain);
    zetui_set_cell (ctx, bx + bw - 1, y + 4,
                    zetui_cell_make (zetui_box_heavy[ZETUI_BOX_RT], plain));
    ...
    zetui_draw_hline (ctx, bx + 1, y + 4, bw - 2, zetui_box_double[ZETUI_BOX_H],
                      plain);
    zetui_set_cell (ctx, bx + bw - 1, y + 4,
                    zetui_cell_make (zetui_box_double[ZETUI_BOX_RT], plain));

    /* -- hlines ---------------------------------------------------- */
    zetui_draw_str (ctx, x, y + 9,
                    "zetui_draw_hline (light / heavy / double):", hdr);
    zetui_draw_hline (ctx, x, y + 10, w - 1, zetui_box_light[ZETUI_BOX_H],
                      plain);
    zetui_draw_hline (ctx, x, y + 11, w - 1, zetui_box_heavy[ZETUI_BOX_H],
                      plain);
    zetui_draw_hline (ctx, x, y + 12, w - 1, zetui_box_double[ZETUI_BOX_H],
                      plain);
    ...
    for (i = 0; i < 3; i++)
        {
            ...
            t = (i == 0)   ? zetui_box_light
                : (i == 1) ? zetui_box_heavy
                           : zetui_box_double;
            zetui_draw_vline (ctx, x + 18 + i * 2, y + 14, 5, t[ZETUI_BOX_V],
                              plain);
        }

    /* -- fill_rect ------------------------------------------------- */
    zetui_draw_str (ctx, x, y + 14, "zetui_fill_rect:",
                    hdr); /* overlaps vline label intentionally as demo */
    zetui_draw_str (ctx, x, y + 20, "zetui_fill_rect:", hdr);
    zetui_fill_rect (ctx, x + 17, y + 20, 18, 3, fill_cell);
    {
        zetui_style_t ts;
        ts = mk_style (ZETUI_COLOR_BRIGHT_WHITE, ZETUI_COLOR_GREEN,
                       ZETUI_ATTR_BOLD);
...
static const char *
key_name (zetui_key_t k)
{
    switch ((int)k)
        {
        case ZETUI_KEY_NONE:
            return "NONE";
        case ZETUI_KEY_CTRL_A:
            return "Ctrl-A";
        case ZETUI_KEY_CTRL_B:
            return "Ctrl-B";
        case ZETUI_KEY_CTRL_C:
            return "Ctrl-C";
        case ZETUI_KEY_CTRL_D:
            return "Ctrl-D";
        case ZETUI_KEY_CTRL_E:
            return "Ctrl-E";
        case ZETUI_KEY_CTRL_F:
            return "Ctrl-F";
        case ZETUI_KEY_CTRL_G:
            return "Ctrl-G";
        case ZETUI_KEY_BACKSPACE:
            return "Backspace";
        case ZETUI_KEY_TAB:
            return "Tab";
        ...
        case ZETUI_KEY_CTRL_X:
            return "Ctrl-X";
        case ZETUI_KEY_CTRL_Y:
            return "Ctrl-Y";
        case ZETUI_KEY_CTRL_Z:
            return "Ctrl-Z";
        case ZETUI_KEY_ESC:
            return "Escape";
        case ZETUI_KEY_DEL:
            return "Del(127)";
        case ZETUI_KEY_ARROW_UP:
            return "Arrow Up";
        case ZETUI_KEY_ARROW_DOWN:
            return "Arrow Down";
        case ZETUI_KEY_ARROW_LEFT:
            return "Arrow Left";
        case ZETUI_KEY_ARROW_RIGHT:
            return "Arrow Right";
        case ZETUI_KEY_HOME:
            return "Home";
        case ZETUI_KEY_END:
            return "End";
        case ZETUI_KEY_PAGE_UP:
            return "Page Up";
        case ZETUI_KEY_PAGE_DOWN:
            return "Page Down";
        case ZETUI_KEY_INSERT:
            return "Insert";
        case ZETUI_KEY_DELETE:
            return "Delete";
        case ZETUI_KEY_F1:
            return "F1";
        case ZETUI_KEY_F2:
...
static void
cp_to_utf8 (zetui_u32 cp, char *buf)
{
    if (cp < 0x80u)
        {
            buf[0] = (char)cp;
...
static void
panel_input (zetui_ctx_t *ctx, int x, int y, int w, int h, zetui_event_t *last,
             int total)
{
    ...
    dim = mk_style (ZETUI_COLOR_BRIGHT_BLACK, ZETUI_COLOR_DEFAULT,
                    ZETUI_ATTR_NONE);
    ...
    if (last->type == ZETUI_EVENT_NONE)
    ...
    else if (last->type == ZETUI_EVENT_KEY)
        {
            ...
            {
                const char *m;
                if (ke->mods & ZETUI_MOD_CTRL)
                    m = "CTRL";
                else if (ke->mods & ZETUI_MOD_ALT)
                    m = "ALT";
                else if (ke->mods & ZETUI_MOD_SHIFT)
                    m = "SHIFT";
                else
                    m = "none";
            ...
            if (ke->ch >= 32u && ke->ch != 127u)
                {
                    ...
                    box_sty = mk_style (ZETUI_COLOR_DEFAULT, ZETUI_COLOR_BLUE,
                                        ZETUI_ATTR_NONE);
                    gly_sty = mk_style (ZETUI_COLOR_BRIGHT_WHITE,
                                        ZETUI_COLOR_BLUE, ZETUI_ATTR_BOLD);

                    zetui_draw_str (ctx, x, row, "glyph:", lbl);
                    zetui_fill_rect (ctx, x + 8, row - 1, 5, 3,
                                     zetui_cell_make ((zetui_u32)' ', box_sty));
    ...
    else if (last->type == ZETUI_EVENT_FOCUS)
        {
            zetui_draw_str (ctx, x, row, "event type  :", lbl);
            zetui_draw_str (ctx, x + 14, row, "FOCUS", val);
            row++;
            zetui_draw_str (ctx, x, row, "focused     :", lbl);
...
static void
panel_utf8 (zetui_ctx_t *ctx, int x, int y, int w, int h)
{
    static const zetui_u32 blocks[]
        = { 0x2588u, 0x2587u, 0x2586u, 0x2585u, 0x2584u, 0x2583u,
            0x2582u, 0x2581u, 0x258Fu, 0x258Eu, 0x258Du, 0x258Cu,
            0x258Bu, 0x258Au, 0x2589u, 0x25A0u };
    static const zetui_u32 misc[]
        = { 0x2665u, 0x2666u, 0x2663u, 0x2660u, 0x263Au, 0x263Bu,
            0x2605u, 0x2606u, 0x2764u, 0x2713u, 0x2717u, 0x2714u,
            0x2190u, 0x2192u, 0x2191u, 0x2193u, 0x21D0u, 0x21D2u,
            0x21D1u, 0x21D3u, 0x00B7u, 0x2022u, 0x25CFu, 0x25CBu };
    static const zetui_u32 braille[]
        = { 0x2800u, 0x2801u, 0x2802u, 0x2803u, 0x2804u, 0x2805u,
            0x2806u, 0x2807u, 0x2840u, 0x2841u, 0x2842u, 0x2843u,
            0x2844u, 0x2845u, 0x2846u, 0x2847u };
    ...
    zetui_draw_str (
        ctx, x, row,
        "All box-drawing codepoints (light / heavy / double):", hdr);
    row++;
    {
        int ti, ci;
        const zetui_u32 *tables[3];
        zetui_i32 fgs[3];
        tables[0] = zetui_box_light;
        fgs[0] = ZETUI_COLOR_CYAN;
        tables[1] = zetui_box_heavy;
        fgs[1] = ZETUI_COLOR_YELLOW;
        tables[2] = zetui_box_double;
        fgs[2] = ZETUI_COLOR_GREEN;
        for (ti = 0; ti < 3; ti++)
            {
                for (ci = 0; ci < 11; ci++)
                    {
                        s = mk_style (fgs[ti], ZETUI_COLOR_DEFAULT,
                                      ZETUI_ATTR_NONE);
                        zetui_set_cell (ctx, x + ci * 2, row,
                                        zetui_cell_make (tables[ti][ci], s));
                    }
                row++;
            }
    }
    ...
    for (col = 0; col < nb; col++)
        {
            s = mk_style (ZETUI_COLOR_BRIGHT_CYAN, ZETUI_COLOR_DEFAULT,
                          ZETUI_ATTR_NONE);
            zetui_set_cell (ctx, x + col * 2, row,
                            zetui_cell_make (blocks[col], s));
        }
    ...
    for (col = 0; col < nb; col++)
        {
            s = mk_style (ZETUI_COLOR_BRIGHT_MAGENTA, ZETUI_COLOR_DEFAULT,
                          ZETUI_ATTR_NONE);
            zetui_set_cell (ctx, x + col * 3, row,
                            zetui_cell_make (misc[col], s));
        }
    ...
    for (col = 0; col < nb; col++)
        {
            s = mk_style (ZETUI_COLOR_BRIGHT_WHITE, ZETUI_COLOR_DEFAULT,
                          ZETUI_ATTR_NONE);
            zetui_set_cell (ctx, x + col * 2, row,
                            zetui_cell_make (braille[col], s));
        }
    ...
    row++;
    s = mk_style (ZETUI_COLOR_BRIGHT_YELLOW, ZETUI_COLOR_DEFAULT,
                  ZETUI_ATTR_NONE);
...
static void
panel_patterns (zetui_ctx_t *ctx, int x, int y, int w, int h)
{
    ...
    zetui_draw_str (ctx, x, row,
                    "fill_rect gradient (8 solid colour bands):", hdr);
    row++;
    for (ci = 0; ci < 8; ci++)
        {
            zetui_style_t fs;
    ...
    inv_sty
        = mk_style (ZETUI_COLOR_YELLOW, ZETUI_COLOR_DEFAULT, ZETUI_ATTR_DIM);
            ...
            for (cx = 0; cx < 32; cx++)
                {
                    ...
                    ps = mk_style ((ci + cx) % 8, ZETUI_COLOR_DEFAULT,
                                   ZETUI_ATTR_NONE);
...
static void
draw_chrome (zetui_ctx_t *ctx, int sel, int total, zetui_event_t *last)
{
    ...
    frame = mk_style (ZETUI_COLOR_BRIGHT_BLACK, ZETUI_COLOR_DEFAULT,
                      ZETUI_ATTR_NONE);
    title_bg
        = mk_style (ZETUI_COLOR_DEFAULT, ZETUI_COLOR_BLUE, ZETUI_ATTR_NONE);
    title_txt = mk_style (ZETUI_COLOR_BRIGHT_WHITE, ZETUI_COLOR_BLUE,
                          ZETUI_ATTR_BOLD);
    tab_sel = mk_style (ZETUI_COLOR_BRIGHT_WHITE, ZETUI_COLOR_BLUE,
                        ZETUI_ATTR_BOLD);
    tab_norm
        = mk_style (ZETUI_COLOR_WHITE, ZETUI_COLOR_DEFAULT, ZETUI_ATTR_NONE);
    dim_sty = mk_style (ZETUI_COLOR_BRIGHT_BLACK, ZETUI_COLOR_DEFAULT,
                        ZETUI_ATTR_NONE);
    ...
    zetui_fill_rect (ctx, 1, 1, w - 2, 1,
                     zetui_cell_make ((zetui_u32)' ', title_bg));
    ...
    /* Row 2: ┣━━━━┳━━━━┫ separator */
    zetui_set_cell (ctx, 0, 2,
                    zetui_cell_make (zetui_box_heavy[ZETUI_BOX_LT], frame));
    zetui_draw_hline (ctx, 1, 2, SIDEBAR_W - 1, zetui_box_heavy[ZETUI_BOX_H],
                      frame);
    zetui_set_cell (ctx, SIDEBAR_W, 2,
                    zetui_cell_make (zetui_box_heavy[ZETUI_BOX_TT], frame));
    zetui_draw_hline (ctx, SIDEBAR_W + 1, 2, w - SIDEBAR_W - 2,
                      zetui_box_heavy[ZETUI_BOX_H], frame);
    zetui_set_cell (ctx, w - 1, 2,
                    zetui_cell_make (zetui_box_heavy[ZETUI_BOX_RT], frame));

    /* Vertical divider between sidebar and content */
    zetui_draw_vline (ctx, SIDEBAR_W, 3, h - 5, zetui_box_heavy[ZETUI_BOX_V],
                      frame);

    /* Row h-2: ┣━━━━┻━━━━┫ separator above status */
    zetui_set_cell (ctx, 0, h - 2,
                    zetui_cell_make (zetui_box_heavy[ZETUI_BOX_LT], frame));
    zetui_draw_hline (ctx, 1, h - 2, SIDEBAR_W - 1,
                      zetui_box_heavy[ZETUI_BOX_H], frame);
    zetui_set_cell (ctx, SIDEBAR_W, h - 2,
                    zetui_cell_make (zetui_box_heavy[ZETUI_BOX_BT], frame));
    zetui_draw_hline (ctx, SIDEBAR_W + 1, h - 2, w - SIDEBAR_W - 2,
                      zetui_box_heavy[ZETUI_BOX_H], frame);
    zetui_set_cell (ctx, w - 1, h - 2,
                    zetui_cell_make (zetui_box_heavy[ZETUI_BOX_RT], frame));
    ...
    while (running)
        {
            ...
            switch (sel)
                {
                case 0:
                    panel_colors (ctx, cx, cy, cw, ch);
                    break;
                case 1:
                    panel_attrs (ctx, cx, cy, cw, ch);
                    break;
                case 2:
                    panel_boxes (ctx, cx, cy, cw, ch);
                    break;
                case 3:
                    panel_input (ctx, cx, cy, cw, ch, &last_ev, total);
                    break;
                case 4:
                    panel_utf8 (ctx, cx, cy, cw, ch);
                    break;
                case 5:
                    panel_patterns (ctx, cx, cy, cw, ch);
                    break;
                default:
                    break;
                }
            ...
            if (sel == 3 && last_ev.type == ZETUI_EVENT_KEY
                && last_ev.data.key.ch >= 32u && last_ev.data.key.ch != 127u)
                {
