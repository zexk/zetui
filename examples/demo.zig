...
fn panelAttrs(tui: *zetui.Context, x: i32, y: i32, w: i32, h: i32) void {
    ...
    tui.drawStr(x, y + 13, "  BOLD | UNDERLINE  (cyan fg)", .{ .fg = .cyan, .attrs = zetui.Attr.bold | zetui.Attr.underline });
...
fn panelBoxes(tui: *zetui.Context, x: i32, y: i32, w: i32, h: i32) void {
    ...
    tui.drawStr(x + 11, y + 21, "filled region", .{ .fg = .bright_white, .bg = .green, .attrs = zetui.Attr.bold });
}
    ...
    switch (last) {
        ...
        .key => |ke| {
            ...
            const mod_str: []const u8 =
                if (ke.mods & zetui.Mod.ctrl != 0) "CTRL" else if (ke.mods & zetui.Mod.alt != 0) "ALT" else if (ke.mods & zetui.Mod.shift != 0) "SHIFT" else "none";
            ...
            if (ke.ch >= 32 and ke.ch != 127) {
                ...
                tui.fillRect(x + 8, row - 1, 5, 3, .{ .style = .{ .bg = .blue } });
                tui.drawSlice(x + 10, row, glyph_s, .{ .fg = .bright_white, .bg = .blue, .attrs = zetui.Attr.bold });
...
fn panelUtf8(tui: *zetui.Context, x: i32, y: i32, w: i32, h: i32) void {
    ...
    row += 1;
    for (blocks, 0..) |cp, i| {
        tui.setCell(x + @as(i32, @intCast(i)) * 2, row, .{ .ch = cp, .style = .{ .fg = .bright_cyan } });
    }
    ...
    row += 1;
    for (misc, 0..) |cp, i| {
        tui.setCell(x + @as(i32, @intCast(i)) * 3, row, .{ .ch = cp, .style = .{ .fg = .bright_magenta } });
    }
    ...
    row += 1;
    for (braille, 0..) |cp, i| {
        tui.setCell(x + @as(i32, @intCast(i)) * 2, row, .{ .ch = cp, .style = .{ .fg = .bright_white } });
    }
...
fn panelPatterns(tui: *zetui.Context, x: i32, y: i32, w: i32, h: i32) void {
    ...
    for (0..8) |ci| {
        const col: zetui.Color = @enumFromInt(ci);
        tui.fillRect(x + @as(i32, @intCast(ci)) * 5, row, 5, 2, .{ .style = .{ .fg = col, .bg = col } });
    }
    row += 3;
    ...
    for (0..3) |i| {
        ...
        tui.drawHline(x, row + @as(i32, @intCast(i)), w - 1, zetui.boxCp(variant, zetui.Box.h), .{ .fg = hl_colors[i] });
    }
    row += 4;
    ...
    tui.drawStr(x + 7, row, "original (green on blue)", .{ .fg = .green, .bg = .blue });
    row += 1;
    tui.drawStr(x, row, "label:", lbl);
    ...
    for (0..6) |ri| {
        for (0..32) |ci| {
            ...
            tui.setCell(x + @as(i32, @intCast(ci)), row + @as(i32, @intCast(ri)), .{ .ch = ch, .style = .{ .fg = fg } });
        }
    }
}
...
fn drawChrome(tui: *zetui.Context, sel: usize, total: i32) void {
    ...
    for (0..NUM_PANELS) |i| {
        ...
        const entry = std.fmt.bufPrint(&buf, " {d} {s:<12}", .{ i + 1, panel_titles[i] }) catch continue;
        if (i == sel)
            tui.fillRect(1, 3 + @as(i32, @intCast(i)) * 2, SIDEBAR_W - 1, 1, .{ .style = ms });
