...
static int zetui__obuf_init(zetui__obuf_t *ob, size_t cap)
{
...
static void zetui__obuf_free(zetui__obuf_t *ob)
{
    free(ob->data);
    ob->data = NULL;
    ob->len  = 0;
...
static int zetui__obuf_append(zetui__obuf_t *ob, const char *data, size_t len)
{
    char  *nd;
    size_t nc;
    ...
    if (ob->len + len > ob->cap) {
        nc = ob->cap * 2;
        if (nc < ob->len + len) nc = ob->len + len + 4096u;
...
static int zetui__obuf_append_str(zetui__obuf_t *ob, const char *s)
{
    return zetui__obuf_append(ob, s, strlen(s));
}

static int zetui__obuf_append_int(zetui__obuf_t *ob, int n)
{
    char tmp[12];
    char buf[13];
    int  i, len;

    if (n == 0) return zetui__obuf_append(ob, "0", 1u);
...
static int zetui__obuf_flush(zetui__obuf_t *ob, int fd)
{
    size_t  written;
    ssize_t n;
...
static void zetui__sigwinch(int sig)
{
    (void)sig;
...
const zetui_u32 zetui_box_heavy[11]  = {
    0x250Fu, 0x2513u, 0x2517u, 0x251Bu,
    0x2501u, 0x2503u,
    0x2523u, 0x252Bu, 0x2533u, 0x253Bu,
    0x254Bu
};

const zetui_u32 zetui_box_double[11] = {
    0x2554u, 0x2557u, 0x255Au, 0x255Du,
    0x2550u, 0x2551u,
    0x2560u, 0x2563u, 0x2566u, 0x2569u,
    0x256Cu
};
...
static size_t zetui__utf8_enc(zetui_u32 cp, char *buf)
{
    if (cp < 0x80u) {
        buf[0] = (char)cp;
        return 1u;
...
static int zetui__utf8_dec(const unsigned char *buf, size_t len, zetui_u32 *out)
{
    if (len == 0u) { *out = 0u; return 0; }
...
static int zetui__ansi_move(zetui__obuf_t *ob, int x, int y)
{
...
static int zetui__ansi_style(zetui__obuf_t *ob,
                              zetui_i32 fg,    zetui_i32 bg,    zetui_u32 attrs,
                              zetui_i32 *c_fg, zetui_i32 *c_bg, zetui_u32 *c_attrs)
{
    ...
    if (turned_off) need_reset = 1;
    if (fg == ZETUI_COLOR_DEFAULT && *c_fg != ZETUI_COLOR_DEFAULT) need_reset = 1;
    if (bg == ZETUI_COLOR_DEFAULT && *c_bg != ZETUI_COLOR_DEFAULT) need_reset = 1;
...
static zetui_cell_t zetui__blank_cell(void)
{
    zetui_cell_t c;
    c.ch    = (zetui_u32)' ';
...
static int zetui__cells_eq(zetui_cell_t a, zetui_cell_t b)
{
    return a.ch == b.ch && a.fg == b.fg && a.bg == b.bg && a.attrs == b.attrs;
}

static int zetui__screen_resize(zetui_ctx_t *ctx, int w, int h)
{
    zetui_cell_t *nf, *nb;
    zetui_cell_t  blank;
    size_t        n, i;
    ...
    if (!nf || !nb) { free(nf); free(nb); return -1; }

    blank = zetui__blank_cell();
    for (i = 0u; i < n; i++) { nf[i] = blank; nb[i] = blank; }
...
zetui_error_t zetui_present(zetui_ctx_t *ctx)
{
    ...
    for (y = 0; y < ctx->height; y++) {
        for (x = 0; x < ctx->width; x++) {
            ...
            if (zetui__cells_eq(*b, *f)) continue;
            ...
            zetui__ansi_style(&ctx->out,
                              b->fg, b->bg, b->attrs,
                              &ctx->ren_fg, &ctx->ren_bg, &ctx->ren_attrs);

            ulen = zetui__utf8_enc(b->ch == 0u ? (zetui_u32)' ' : b->ch, utf8);
    ...
    return zetui__obuf_flush(&ctx->out, ctx->fd_out) == 0
        ? ZETUI_OK : ZETUI_ERR_IO;
}
...
static int zetui__enter_raw(int fd, struct termios *saved)
{
    struct termios raw;

    if (tcgetattr(fd, saved) == -1) return -1;
...
static void zetui__leave_raw(int fd, const struct termios *saved)
{
    tcsetattr(fd, TCSAFLUSH, saved);
}

static int zetui__query_size(int fd, int *w, int *h)
{
    struct winsize ws;
...
zetui_ctx_t *zetui_init(void)
{
    ...
    if (!isatty(ctx->fd_out)) { free(ctx); return NULL; }
...
int zetui_width(const zetui_ctx_t *ctx)  { return ctx->width;  }
int zetui_height(const zetui_ctx_t *ctx) { return ctx->height; }

/* ------------------------------------------------------------------ */
/*  Cursor                                                             */
/* ------------------------------------------------------------------ */

void zetui_cursor_hide(zetui_ctx_t *ctx) { ctx->cursor_visible = 0; }
void zetui_cursor_show(zetui_ctx_t *ctx) { ctx->cursor_visible = 1; }
void zetui_cursor_move(zetui_ctx_t *ctx, int x, int y)
{
    ctx->cursor_x = x;
    ctx->cursor_y = y;
...
static int zetui__ibuf_fill(zetui__ibuf_t *ib, int fd, int timeout_ms)
{
    fd_set         rfds;
    struct timeval tv;
    struct timeval *tvp;
...
static zetui_event_t zetui__parse(zetui__ibuf_t *ib)
{
    zetui_event_t              ev;
    unsigned char              c;
    ...
    if (c == 0x1Bu && ib->len - ib->pos > 1) {
        ...
        for (e = zetui__esc_table; e->seq != NULL; e++) {
            ...
            if (slen <= rem && memcmp(buf, e->seq, (size_t)slen) == 0) {
...
static zetui_event_t zetui__resize_ev(zetui_ctx_t *ctx)
{
    zetui_event_t ev;
    int           w, h;
...
zetui_event_t zetui_poll_event(zetui_ctx_t *ctx)
{
    ...
    if (zetui__resize_flag) return zetui__resize_ev(ctx);
