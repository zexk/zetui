...
- **Input** — keyboard (special keys + modifiers + Shift+Tab), mouse (SGR, all-motion with distinct hover vs drag), resize (SIGWINCH), focus, bracketed paste
- **Colors** — 16-color ANSI, 256-color xterm palette (`ZETUI_COLOR_256`), 24-bit RGB truecolor
- **Styled underlines** — single, double, curly, dotted, dashed; independent underline color via SGR 58
- **Box drawing** — light / heavy / double Unicode line characters
- **Drawing helpers** — column-clipped draw (`zetui_draw_str_len`), printf-style draw (`zetui_draw_printf`), clipped printf (`zetui_draw_printf_len`)
- **OSC 8 hyperlinks** — clickable text; heap-managed URI table; hover hit-test via `zetui_get_link_at`; reverse URI lookup via `zetui_get_link_uri`
...
- **Terminal title** — OSC 2 window title; OSC 1 icon/taskbar title
- **Clipboard** — OSC 52 clipboard write via `zetui_set_clipboard`
- **Suspend / resume** — `zetui_suspend()` / `zetui_resume()` for SIGTSTP (Ctrl-Z) support
